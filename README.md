...
![version](https://img.shields.io/badge/version-1.16.2-7c8cf8) ![manifest](https://img.shields.io/badge/Chrome-MV3-4caf50) ![node](https://img.shields.io/badge/node-%3E%3D18-339933) ![deps](https://img.shields.io/badge/dependencies-0-brightgreen) ![license](https://img.shields.io/badge/license-MIT-blue)
...
- 🎨 **Canvas 简历读取（结构化全文，零 OCR）** — `readResumeCanvasCdp()`（MCP `browser_read_canvas_text`）用 `chrome.debugger` 派发**可信滚轮**逐屏驱动 canvas 重画，边滚边记偏移，重建出从页首到页尾的完整简历。攻克 Boss 这类“JS 拦截滚轮、无 DOM 滚动、合成事件被 isTrusted 挡掉”的反爬 canvas。需 `debugger` 权限；读取时本地 Chrome 短暂显示调试横幅、简历自动滚动几秒。备选：`readCanvasImage()`/`readCanvasFull()` 导出图片交视觉模型 OCR。
...
**Canvas 简历读到的是空的 / 乱码？**
- 用 `read_resume_canvas_cdp`（MCP `browser_read_canvas_text`，v1.16.15+）：打开候选人在线简历弹窗后直接调，它用可信滚轮逐屏读全文。别用只读缓冲的 `mode:"static"`——视口大小、滚动重画的 canvas 只读缓冲会不全/交织。
- 前提：扩展是含 `canvas-hook.js` + `debugger` 权限的 1.16.15+，且已授予 debugger 权限。`browser_canvas_diag` 可查钩子是否生效（`hookInstalled`/`capturedDraws`）。
- **重载扩展后记得也刷新控制台页**（否则中继还是旧 content.js，握手版本对不上、命令可能落到旧代码）。

**网络抓包读到的是空的？**
- 需 v1.4.0+（钩子改在页面**主世界**注入才生效）。
